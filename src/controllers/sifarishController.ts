...
import { Sifarish } from "../entities/cadastral/Sifarish";
import { Cadastral } from "../entities/cadastral/Cadastral";
import HttpError from "../util/httpError";
import { User } from "../entities/user/User";
import { Raw } from "typeorm";

class SifarishController {
  constructor(
    private sifarishRepo = AppDataSource.getRepository(Sifarish),
  ...
  async createSifarish(
    ...
    try {
      ...
      await this.sifarishRepo.save(sifarish);

      return res.status(200).json({
        status: 201,
        message: "Sifarish generated sucessfully.",
        data: sifarish,
      });
    } catch (error) {
      console.error(error);
  ...
  async getSifarishByUuid(
    ...
    try {
      const sifarish = await this.sifarishRepo.findOne({
        where: { uuid },
  ...
  async toggleValidState(req: Request, res: Response, next: NextFunction) {
    try {
      const { id } = req.params;
      const sifarish = await this.sifarishRepo.findOneBy({ id: Number(id) });
      if (!sifarish) {
  ...
  async deleteSifarish(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      await this.sifarishRepo.remove(sifarish);
  ...
  async getSifarish(
    ...
    try {
      ...
      let queryBuilder = this.sifarishRepo
...
export default new SifarishController();
